...
typedef void(LIBZENAPI *_ZenSetAmafWeightFactor)(float);
typedef void(LIBZENAPI *_ZenSetBoardSize)(int);
typedef void(LIBZENAPI *_ZenSetDCNN)(bool);
...
typedef void(LIBZENAPI *_ZenSetPriorWeightFactor)(float);
...
struct _zen_dll_proxy {
	...
	_ZenSetAmafWeightFactor ZenSetAmafWeightFactor;
	_ZenSetBoardSize ZenSetBoardSize;
	_ZenSetDCNN ZenSetDCNN;
	...
	_ZenSetPriorWeightFactor ZenSetPriorWeightFactor;
...
CZen6Gtp::CZen6Gtp() : m_proxy(NULL)
{
}
...
bool CZen6Gtp::load(std::string zen_dll_path, std::string lua_engine_path)
{
#if 0
	...
	((_zen_dll_proxy*)m_proxy)->ZenSetAmafWeightFactor = (_ZenSetAmafWeightFactor)GetProcAddress(hModule, "?ZenSetAmafWeightFactor@@YAXM@Z");
	assert(((_zen_dll_proxy*)m_proxy)->ZenSetAmafWeightFactor);
	...
	((_zen_dll_proxy*)m_proxy)->ZenSetDCNN = (_ZenSetDCNN)GetProcAddress(hModule, "?ZenSetDCNN@@YAX_N@Z");
	assert(((_zen_dll_proxy*)m_proxy)->ZenSetDCNN);
	...
	((_zen_dll_proxy*)m_proxy)->ZenSetPriorWeightFactor = (_ZenSetPriorWeightFactor)GetProcAddress(hModule, "?ZenSetPriorWeightFactor@@YAXM@Z");
	assert(((_zen_dll_proxy*)m_proxy)->ZenSetPriorWeightFactor);
	...
	if (!((_zen_dll_proxy*)m_proxy)->ZenSetDCNN || !((_zen_dll_proxy*)m_proxy)->ZenSetPriorWeightFactor) {
		fprintf(stderr, "zen.dll not copy from zen6 version?\n");
		return false;
	}
	...
	((_zen_dll_proxy*)m_proxy)->ZenSetAmafWeightFactor(1.0);
	((_zen_dll_proxy*)m_proxy)->ZenSetPriorWeightFactor(1.0);
	((_zen_dll_proxy*)m_proxy)->ZenSetDCNN(true);
...
std::string CZen6Gtp::list_commands()
{
...
std::string CZen6Gtp::protocol_version()
{
	return "= 2\n";
}

std::string CZen6Gtp::clear_board()
{
...
std::string CZen6Gtp::boardsize(int size)
{
...
// play b q16
// play w q3
std::string CZen6Gtp::play(std::string _color, std::string position)
{
...
std::string CZen6Gtp::__find_best_move(bool debug, int &x, int &y, int &simulation, float &W)
{
	// 寻找最优
	int N = -1;
...
// 指定最大时间（毫秒），最多步数
std::string CZen6Gtp::__genmove(std::string _color, int _maxtime, int _strength)
{
...
std::string CZen6Gtp::genmove(std::string _color)
{
...
std::string CZen6Gtp::time_settings(int main_time, int byo_yomi_time, int byo_yomi_stones)
{
...
std::string CZen6Gtp::time_left(std::string color, int time, int stones)
{
